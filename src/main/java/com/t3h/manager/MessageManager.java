...
import com.t3h.model.request.SendMessageRequest;
import com.t3h.model.response.MessageResponse;
import com.t3h.model.response.ResponseUtils;
import com.t3h.mysql.tables.Message;
import com.t3h.mysql.tables.User;
import com.t3h.mysql.tables.records.MessageRecord;
import com.t3h.security.withoutloginregister.JwtAuthenticationToken;
...
public class MessageManager extends BaseManager {
    ...
    public Object sendMessage(SendMessageRequest request) {
        ...
        String senderId = dslContext.select(User.USER.USER_ID)
                .from(User.USER)
    ...
    public Object messages() {
        ...
        String username = token.getCredentials().getUsername();
        String userId = (dslContext.select(User.USER.USER_ID)
                .from(User.USER)
                .where(User.USER.USERNAME.eq(username))
                .fetchAny()
                .value1());
